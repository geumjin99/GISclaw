...
- Chinese and Korean interfaces use fonts that carry their own script in every role, with sizes and spacing suited to them.
- The interface in English, Chinese and Korean: a language button at the right of the menu bar; the choice is remembered on this machine. The agent's closing summary, the closing note and the project log follow the interface language.
- Settings → Map: choose the basemap — Esri (street by default; light gray, topographic, imagery) or OpenTopoMap without a key, MapTiler / Mapbox / Thunderforest with one, any XYZ template of your own, an MBTiles file for fully offline use, or none. Tiles are fetched by GISclaw and cached in the data folder, so a key never reaches the page and viewed areas stay available offline. A built-in Natural Earth layer (land, lakes, borders) is always drawn underneath.
