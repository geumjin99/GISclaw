...
4. geoprocess(op, inputs, params, output, save_as)
   Run a BUILT-IN deterministic geoprocessing operation on
   variables already in the environment. PREFER this over writing code for standard
   operations — it is reliable, CRS-aware, and auto-renders vector results on the map.
   - op: operation id (see catalog below)
   - inputs: {{role: var_name}} mapping op input roles to loaded variables
     (e.g. {{"layer": "roads"}}, {{"zones": "blocks", "raster": "dem"}})
   - params: operation parameters (e.g. {{"distance": 100}})
   - output: name to bind the result to in the environment
   - save_as: optional path to also persist the result (e.g. "pred_results/out.geojson")
   Available operations:
{geoprocess_ops}

5. execute(code)
   Execute a code snippet in a persistent Python environment. Variables persist across calls.
   Use this ONLY for analysis not covered by a geoprocess op (custom formulas, bespoke
   modelling, visualization/plots). Returns: stdout + stderr + new variable summary.
...
7. finish(summary)
   Mark task as complete. Provide a result summary.""".format(
            geoprocess_ops=geo_ops.describe_ops())
    ...
    def list_files(self) -> str:
        ...
        if not os.path.exists(data_path):
            return f"❌ Data directory does not exist: {self.data_dir}/"
        
        files = sorted(os.listdir(data_path))
        if not files:
            return f"Data directory {self.data_dir}/ is empty"
        
        lines = [f"📁 {self.data_dir}/ directory ({len(files)} files):"]
        for fname in files:
...
"""
    ...
    def geoprocess(self, op: str, inputs: Dict[str, Any] = None,
                   params: Dict[str, Any] = None, output: str = None,
                   save_as: str = None) -> str:
        """Run a built-in deterministic geoprocessing operation.

        Generates a fixed, tested code template from geo_ops.REGISTRY and runs it in
        the persistent sandbox. Inputs/outputs are sandbox variables; vector results
        auto-write a WGS84 GeoJSON to pred_results/ so the map renders them.
        """
        if op not in geo_ops.REGISTRY:
            avail = ", ".join(sorted(geo_ops.REGISTRY.keys()))
            return f"❌ Unknown geoprocess op '{op}'. Available: {avail}"
        if not output:
            return "❌ geoprocess requires an 'output' variable name."
        if not output.isidentifier():
            return f"❌ Invalid output name '{output}' (must be a valid Python identifier)."
        try:
            code = geo_ops.build_code(op, inputs, params, output, save_as)
        except KeyError as e:
            need = geo_ops.REGISTRY[op].get("params", "")
            return (f"❌ geoprocess '{op}' is missing a required input/param: {e}. "
                    f"Expected: {need}")
        result = self.sandbox.execute(code)
        obs = result.to_observation()
        # Surface the op label so the trace reads cleanly
        return f"[geoprocess {op} → {output}]\n{obs}"
            ...
            for f in os.listdir(pred_dir):
                ...
                if f not in before_state or mtime > before_state.get(f, 0):
                    ...
                    if ext == '.png' and size_kb < 30:
                        warnings.append(
                            f"⚠️ Output QC warning: {f} is only {size_kb:.1f}KB, "
    ...
    def finish(self, summary: str = "") -> str:
        ...
        if output_files:
            ...
            result += "⚠️ No output files generated! Please ensure results are saved to pred_results/\n"
